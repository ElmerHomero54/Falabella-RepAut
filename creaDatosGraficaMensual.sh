...
# --
# -- Calcula el rango de fechas para 12 meses (nm)
rango=$(echo $ODATE |
         awk -v nm=12 '{ if(nm==12) nm=1
            finis=$0; yr_f=substr(finis,1,4); mt_f=substr(finis,5,2)
            yr_i = yr_f - 1; mt_i = mt_f + 1; if(mn_i<0) mn_i=mn_i+nm
            printf("%04d%02d@%04d%02d\n", yr_i,mt_i,yr_f,mt_f) }')
# --
# -- Separa los limites inicial y final del rango calculado
mi=$(echo $rango | cut -d'@' -f 1)
mf=$(echo $rango | cut -d'@' -f 2)
...
cat $DAT/t1_$pais.txt |
   ...
   sed 's/.$//' > $DAT/tmpMensual_$pais.txt
# --
      ...
      awk -F'@' -v nm=12 -v odate=$ODATE '
         BEGIN{s=FS
            ...
            for(i=1;i<=nm;i++) {
               ...
               mt=mt-1;if(mt==0) { mt=12; yr=yr-1 } } }
         { a[$1]=$2 }
         END{for(i in a) printf("%s@",a[i]);printf("\n")}' >> $DAT/tmpMensual_$pais.txt
   # ---
done

#END{for(i in ji) if(ji[i]!="" && jf[i]!="") print i s strftime("%H%M%S", jf[i] - ji[i]) #jf[i] - ji[i] }'